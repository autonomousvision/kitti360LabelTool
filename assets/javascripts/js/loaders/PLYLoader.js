...
THREE.PLYLoader.prototype = {
	...
	load: function ( url, callback ) {
		...
		request.addEventListener( 'load', function ( event ) {
			...
			var plyObj = scope.parse( event.target.response );

			scope.dispatchEvent( { type: 'load', content: plyObj} );
	...
	parseASCII: function ( data ) {
		...
		for ( var i = 0; i < lines.length; i ++ ) {
			...
			this.handleElement( geometry, zvar, timestamp, cartoonColor, header.elements[currentElement].name, element );
		...
		var plyObj = {
			Geometry: this.postProcess( geometry ),
	...
	handleElement: function ( geometry, zvar, timestamp, cartoonColors, elementName, element ) {
		...
		if ( elementName === "vertex" ) {
			...
			if ( 'red_ex' in element && 'green_ex' in element && 'blue_ex' in element ) {
				var color_elem = new THREE.Color();
				color_elem.setRGB( element.red_ex / 255.0, element.green_ex / 255.0, element.blue_ex / 255.0 );
				cartoonColors.push( color_elem );
			}

			if ('zvar' in element) {
				zvar.push(element.zvar);
			}
